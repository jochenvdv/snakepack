...
class RequestsAcceptanceTest(BaseAcceptanceTest):
    _SUBJECT_NAME = 'requests'
    _SOURCEDIR = Path(__file__).resolve().parent / 'subjects' / _SUBJECT_NAME
    _LIBRARY_PACKAGES = ['requests']
    _PRETEST_CMD = f'{{venv_path}}/pip install -r {str(_SOURCEDIR)}/requirements-dev.txt'
